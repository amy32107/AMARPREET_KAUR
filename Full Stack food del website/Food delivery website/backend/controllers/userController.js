...
const registerUser = async (req,res) => {
    //logic to allow user to  register using the website
    const {name,password,email} = req.body;
    try {
        const exists = await userModel.findOne({email}); //if email available for any account then that account will be stored in that variable
        if(exists)
        {
